...
test.describe("The /replays discovery page", () => {
  ...
  test("the collapsible panel exposes the full filter control set", async ({ page }) => {
    ...
    // The toggle input is visually hidden behind its styled switch; assert
    // on the testid (visibility of the styled control's input wrapper).
    await expect(page.getByTestId("filter-rated-only")).toBeAttached();

    // exact: true — role-name matching is substring-based and "Done" would
    // also match the open panel's "Abandoned" result pill.
